...
public:
	...
	void update(char key) {
		if (isWritable_) {
			if (size_ < maxLimit_ && size_ < MAX_SIZE && isValidKey(key,marker_)) {
...
protected:
	virtual bool isValidKey(char key, int index) const {
		if (key == ' ') {
			return true;
		}
		if (key >= 'a' && key <= 'z') {
	...
	virtual bool isRemovable(int index) const {
		return true;
	}
...
typedef std::shared_ptr<gui::InputFormatter> InputFormatterPtr;
