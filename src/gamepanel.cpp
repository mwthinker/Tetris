...
GamePanel::GamePanel(TetrisGame& tetrisGame)
	: tetrisGame_(tetrisGame), alivePlayers_(0),
	updateMatrix_(true), buffer_(mw::Buffer::STATIC) {

	setGrabFocus(true);
	eventConnection_ = tetrisGame_.addGameEventHandler(std::bind(&GamePanel::eventHandler, this, std::placeholders::_1));
...
void GamePanel::validate() {
	Panel::validate();
	updateMatrix_ = true;
}

void GamePanel::init() {
	add<Button>(0, "hej", TetrisData::getInstance().getDefaultFont(20));
	auto comboBox = add<ComboBox>(1, TetrisData::getInstance().getDefaultFont(20));
	comboBox->addItem("Keyboard 1");
	comboBox->setSelectedItem(0);
	comboBox->addItem("Keyboard 2");
	comboBox->addItem("Default AI");
}

void GamePanel::draw(const gui::Graphic& graphic, double deltaTime) {
	...
	gui::Panel::draw(graphic, deltaTime);
	mw::checkGlError();
}

void GamePanel::initGame(std::vector<PlayerPtr>& players) {
...
void GamePanel::countDown(int msCountDown) {
...
void GamePanel::eventHandler(TetrisGameEvent& tetrisEvent) {
	try {
...
void GamePanel::soundEffects(GameEvent gameEvent) const {
	mw::Sound sound;
	switch (gameEvent) {
