...
The game uses OpenGL and all source code can run on multiple platforms 
(e.g. Windows, Linux and MAC).

Player 1: Is controlled by the arrows key on keyboard.
Player 2: Is controlled by the (a,s,d,w) keys on keyboard.
Player 3-4: Is controlled by any gamepad connected to the computer.

The network uses UDP (not TCP) so the chosen port may has to be 
...
Developed by MwThinker (github), year 2014.
